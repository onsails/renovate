...
async function writeExistingFiles(config, packageFiles) {
    ...
    if (config.npmrc) {
        ...
        await fs_1.outputFile(npmrcFile, `${String(config.npmrc)}\n`);
    }
    ...
    for (const packageFile of npmFiles) {
        const basedir = upath_1.default.join(config.localDir, upath_1.default.dirname(packageFile.packageFile));
...
async function updateYarnOffline(lockFileDir, localDir, updatedArtifacts) {
    try {
        const resolvedPaths = [];
        const yarnrcYml = await git_1.getFile(upath_1.default.join(lockFileDir, '.yarnrc.yml'));
        ...
        if (yarnrcYml) {
            ...
            const config = parsers_1.parseSyml(yarnrcYml);
            resolvedPaths.push(upath_1.default.join(lockFileDir, config.cacheFolder || './.yarn/cache'));
            resolvedPaths.push(upath_1.default.join(lockFileDir, '.pnp'));
            if (config.pnpDataPath) {
                resolvedPaths.push(upath_1.default.join(lockFileDir, config.pnpDataPath));
            }
        }
        else if (yarnrc) {
...
async function getAdditionalFiles(config, packageFiles) {
    ...
    if (config.updateType === 'lockFileMaintenance' &&
        ...
        git_1.branchExists(config.branchName)) {
    ...
    for (const lockFile of dirs.npmLockDirs) {
        const lockFileDir = upath_1.default.dirname(lockFile);
        ...
        const fileName = upath_1.default.basename(lockFile);
    ...
    for (const lernaDir of dirs.lernaDirs) {
        ...
        const lernaPackageFile = packageFiles.npm.find((p) => upath_1.default.dirname(p.packageFile) === lernaDir);
        if (!lernaPackageFile) {
