...
function resolveParentFile(packageFile, parentPath) {
    ...
    const parentBasename = upath_1.basename(parentPath);
    if (parentBasename === 'pom.xml' || parentBasename.endsWith('.pom.xml')) {
        parentFile = parentBasename;
        parentDir = upath_1.dirname(parentPath);
    }
    const dir = upath_1.dirname(packageFile);
    return upath_1.normalize(upath_1.join(dir, parentDir, parentFile));
}
