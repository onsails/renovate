...
describe('regex', () => {
    const regex = __1.get('regex:^(?<major>\\d+)\\.(?<minor>\\d+)\\.(?<patch>\\d+)(?<prerelease>[^.-]+)?(?:-(?<compatibility>.*))?$');
    it('requires a valid configuration to be initialized', () => {
        expect(() => __1.get('regex:not a regex')).toThrow();
    });
    describe('throws', () => {
        for (const re of [
            '^(?<major>\\d+)(',
            ...
            it(re, () => {
                expect(() => __1.get(`regex:${re}`)).toThrow(error_messages_1.CONFIG_VALIDATION);
            });
        }
    });
    describe('.parse()', () => {
    ...
    describe('.getSatisfyingVersion', () => {
        it('returns greatest version that matches range', () => {
            expect(regex.getSatisfyingVersion(['2.1.5', '2.1.6a1', '2.1.6', '2.1.6-foo'], '2.1.6')).toEqual('2.1.6');
            expect(regex.getSatisfyingVersion(['2.1.5', '2.1.6a1', '2.1.6', '2.1.6-foo'], '2.1.6-foo')).toEqual('2.1.6');
            expect(regex.getSatisfyingVersion(['2.1.5-foo', '2.1.6'], '2.1.6-foo')).toEqual('2.1.6');
        });
        it('returns null if version that matches is absent', () => {
            expect(regex.getSatisfyingVersion(['1.2.3', '1.2.4'], '3.5.0')).toBeNull();
        });
    });
    describe('.minSatisfyingVersion', () => {
        it('returns least version that matches range', () => {
            expect(regex.minSatisfyingVersion(['2.1.5', '2.1.6a1', '2.1.6', '2.1.6-foo'], '2.1.6')).toEqual('2.1.6');
            expect(regex.minSatisfyingVersion(['2.1.5', '2.1.6a1', '2.1.6', '2.1.6-foo'], '2.1.6-foo')).toEqual('2.1.6');
            expect(regex.minSatisfyingVersion(['2.1.5', '2.1.6-foo'], '2.1.5-foo')).toEqual('2.1.5');
        });
        it('returns null if version that matches is absent', () => {
            expect(regex.minSatisfyingVersion(['1.2.3', '1.2.4'], '3.5.0')).toBeNull();
        });
    });
    describe('.getNewValue', () => {
        it('returns toVersion', () => {
            expect(regex.getNewValue({
                currentValue: null,
                rangeStrategy: null,
                fromVersion: null,
                toVersion: '1.2.3',
    ...
    describe('.sortVersions', () => {
        it('sorts versions in an ascending order', () => {
            expect(['1.2.3a1', '2.0.1', '1.3.4', '1.2.3'].sort(regex.sortVersions.bind(regex))).toEqual(['1.2.3a1', '1.2.3', '1.3.4', '2.0.1']);
        });
    });
    describe('.matches', () => {
