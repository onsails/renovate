...
const renovate_config_ikatyang_json_1 = __importDefault(require("./__fixtures__/renovate-config-ikatyang.json"));
const _npm = __importStar(require("./npm"));
const presets = __importStar(require("."));
jest.mock('./npm');
jest.mock('./github');
const npm = util_1.mocked(_npm);
npm.getPreset = jest.fn(({ packageName, presetName }) => {
    if (packageName === 'renovate-config-ikatyang') {
        return renovate_config_ikatyang_json_1.default.versions[renovate_config_ikatyang_json_1.default['dist-tags'].latest]['renovate-config'][presetName];
    }
...
describe('config/presets', () => {
    ...
    describe('parsePreset', () => {
        ...
        it('parses github subfiles', () => {
            expect(presets.parsePreset('github>some/repo:somefile')).toMatchSnapshot();
        });
        it('parses github subfiles with preset name', () => {
            expect(presets.parsePreset('github>some/repo:somefile/somepreset')).toMatchSnapshot();
        });
        it('parses github subfiles with preset and sub-preset name', () => {
            expect(presets.parsePreset('github>some/repo:somefile/somepreset/somesubpreset')).toMatchSnapshot();
        });
        it('parses gitlab', () => {
        ...
        it('returns default package name with params', () => {
            expect(presets.parsePreset(':group(packages/eslint, eslint)')).toMatchSnapshot();
        });
        // scoped namespace
        ...
        it('returns scope with packageName and default', () => {
            expect(presets.parsePreset('@somescope/somepackagename')).toMatchSnapshot();
        });
        it('returns scope with packageName and params and default', () => {
            expect(presets.parsePreset('@somescope/somepackagename(param1, param2, param3)')).toMatchSnapshot();
        });
        it('returns scope with presetName', () => {
            expect(presets.parsePreset('@somescope:somePresetName')).toMatchSnapshot();
        });
        it('returns scope with presetName and params', () => {
            expect(presets.parsePreset('@somescope:somePresetName(param1)')).toMatchSnapshot();
        });
        it('returns scope with packageName and presetName', () => {
            expect(presets.parsePreset('@somescope/somepackagename:somePresetName')).toMatchSnapshot();
        });
        it('returns scope with packageName and presetName and params', () => {
            expect(presets.parsePreset('@somescope/somepackagename:somePresetName(param1, param2)')).toMatchSnapshot();
        });
        ...
        it('returns non-scoped package name full', () => {
            expect(presets.parsePreset('renovate-config-somepackage:webapp')).toMatchSnapshot();
        });
        it('returns non-scoped package name with params', () => {
            expect(presets.parsePreset('somepackage:webapp(param1)')).toMatchSnapshot();
        });
    });
    describe('getPreset', () => {
