...
describe(util_1.getName(__filename), () => {
    ...
    describe('fetchJSONFile()', () => {
        it('returns JSON', async () => {
            ...
            const res = await github.fetchJSONFile('some/repo', 'some-filename.json', githubApiHost);
    ...
    describe('getPreset()', () => {
        it('tries default then renovate', async () => {
            httpMock
                ...
                .reply(200, {});
            await expect(github.getPreset({ packageName: 'some/repo' })).rejects.toThrow();
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('throws if no content', async () => {
            httpMock
                ...
                .get(`${basePath}/default.json`)
                .reply(200, {});
            await expect(github.getPreset({ packageName: 'some/repo' })).rejects.toThrow('invalid preset JSON');
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    ...
    describe('getPresetFromEndpoint()', () => {
        it('uses default endpoint', async () => {
            ...
            expect(await github.getPresetFromEndpoint('some/repo', 'default')).toEqual({ from: 'api' });
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('uses custom endpoint', async () => {
            ...
            expect(await github
                .getPresetFromEndpoint('some/repo', 'default', 'https://api.github.example.org')
