...
describe('config/migration', () => {
    describe('migrateConfig(config, parentConfig)', () => {
        it('migrates config', () => {
            ...
            const { isMigrated, migratedConfig } = configMigration.migrateConfig(config, parentConfig);
        ...
        it('migrates before and after schedules', () => {
            ...
            expect(migratedConfig.minor.schedule[0]).toEqual('after 10pm every weekday');
            expect(migratedConfig.minor.schedule[1]).toEqual('before 7am every weekday');
        });
        ...
        it('does not migrate config', () => {
            ...
            const { isMigrated, migratedConfig } = configMigration.migrateConfig(config);
            expect(isMigrated).toBe(false);
        ...
        it('migrates subconfig', () => {
            ...
            const { isMigrated, migratedConfig } = configMigration.migrateConfig(config, defaultConfig);
            expect(isMigrated).toBe(true);
            expect(migratedConfig).toMatchSnapshot();
            expect(migratedConfig.lockFileMaintenance.packageRules).toHaveLength(1);
            expect(migratedConfig.lockFileMaintenance.packageRules[0].respectLatest).toBe(false);
        });
        ...
        it('it migrates unpublishSafe', () => {
            ...
            config = { unpublishSafe: true, extends: 'foo' };
            res = configMigration.migrateConfig(config);
            expect(res.isMigrated).toBe(true);
            expect(res.migratedConfig).toMatchObject({
