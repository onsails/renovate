...
const aalib = fs_1.default.readFileSync('lib/manager/homebrew/__fixtures__/aalib.rb', 'utf8');
const aap = fs_1.default.readFileSync('lib/manager/homebrew/__fixtures__/aap.rb', 'utf8');
const acmetool = fs_1.default.readFileSync('lib/manager/homebrew/__fixtures__/acmetool.rb', 'utf8');
const aide = fs_1.default.readFileSync('lib/manager/homebrew/__fixtures__/aide.rb', 'utf8');
const ibazel = fs_1.default.readFileSync('lib/manager/homebrew/__fixtures__/ibazel.rb', 'utf8');
describe('lib/manager/homebrew/extract', () => {
    describe('extractPackageFile()', () => {
        it('skips sourceforge dependency 1', () => {
            const res = extract_1.extractPackageFile(aalib);
            expect(res).not.toBeNull();
            expect(res.deps[0].skipReason).toBe('unsupported-url');
            expect(res).toMatchSnapshot();
        });
        it('skips sourceforge dependency 2', () => {
            const res = extract_1.extractPackageFile(aap);
            expect(res).not.toBeNull();
            expect(res.deps[0].skipReason).toBe('unsupported-url');
            expect(res).toMatchSnapshot();
        });
        it('skips github dependency with wrong format', () => {
            const res = extract_1.extractPackageFile(acmetool);
            expect(res).not.toBeNull();
            expect(res.deps[0].skipReason).toBe('unsupported-url');
            expect(res).toMatchSnapshot();
        });
        it('extracts "releases" github dependency', () => {
            const res = extract_1.extractPackageFile(aide);
            expect(res).not.toBeNull();
            expect(res.deps[0].skipReason).toBeUndefined();
            expect(res).toMatchSnapshot();
        });
        it('extracts "archive" github dependency', () => {
            const res = extract_1.extractPackageFile(ibazel);
            expect(res).not.toBeNull();
            expect(res.deps[0].skipReason).toBeUndefined();
            expect(res).toMatchSnapshot();
        });
      ...
      `;
            expect(extract_1.extractPackageFile(content)).toBeNull();
        });
