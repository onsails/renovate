...
describe('manager/homebrew/update', () => {
    ...
    it('updates "releases" github dependency', async () => {
        ...
        httpMock
            ...
            .reply(200, stream_1.Readable.from(['foo']));
        const newContent = await update_1.updateDependency({
            fileContent: aide,
            upgrade,
        });
    ...
    it('updates "archive" github dependency', async () => {
        ...
        httpMock
            .scope(baseUrl)
            .get('/bazelbuild/bazel-watcher/releases/download/v0.9.3/bazel-watcher-0.9.3.tar.gz')
            .reply(200, stream_1.Readable.from(['foo']));
        const newContent = await update_1.updateDependency({
            fileContent: ibazel,
            upgrade,
    ...
    it('returns unchanged content if repoName in upgrade object is invalid', async () => {
        ...
        httpMock
            .scope(baseUrl)
            .get('/bazelbuild/invalid/repo/name/releases/download/v0.9.3/invalid/repo/name-0.9.3.tar.gz')
            .replyWithError('')
    ...
    it('returns unchanged content if repoName in upgrade object is wrong', async () => {
        ...
        httpMock
            .scope(baseUrl)
            .get('/bazelbuild/wrong-version/archive/v10.2.3.tar.gz/releases/download/v0.9.3/wrong-version/archive/v10.2.3.tar.gz-0.9.3.tar.gz')
            .replyWithError('')
            .get('/bazelbuild/wrong-version/archive/v10.2.3.tar.gz/archive/v0.9.3.tar.gz')
            .reply(200, stream_1.Readable.from(['foo']));
        const newContent = await update_1.updateDependency({
            fileContent: content,
            upgrade,
