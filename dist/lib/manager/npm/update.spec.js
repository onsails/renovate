...
const npmUpdater = __importStar(require("./update"));
function readFixture(fixture) {
    return fs_1.default.readFileSync(upath_1.default.resolve(__dirname, `./__fixtures__/${fixture}`), 'utf8');
}
...
describe('workers/branch/package-json', () => {
    describe('.updateDependency(fileContent, depType, depName, newValue)', () => {
        ...
        it('updates glob resolutions without dep', () => {
            ...
            expect(JSON.parse(testContent).resolutions['**/@angular/cli']).toEqual('8.1.0');
        });
    ...
    describe('.bumpPackageVersion()', () => {
        ...
        it('mirrors', () => {
            const { bumpedContent } = npmUpdater.bumpPackageVersion(content, '0.0.2', 'mirror:chalk');
            expect(bumpedContent).toMatchSnapshot();
            expect(bumpedContent).not.toEqual(content);
        });
        it('aborts mirror', () => {
            const { bumpedContent } = npmUpdater.bumpPackageVersion(content, '0.0.2', 'mirror:a');
            expect(bumpedContent).toEqual(content);
        });
        it('increments', () => {
            const { bumpedContent } = npmUpdater.bumpPackageVersion(content, '0.0.2', 'patch');
            expect(bumpedContent).toMatchSnapshot();
            expect(bumpedContent).not.toEqual(content);
        });
        it('no ops', () => {
            const { bumpedContent } = npmUpdater.bumpPackageVersion(content, '0.0.1', 'patch');
            expect(bumpedContent).toEqual(content);
        });
        it('updates', () => {
            const { bumpedContent } = npmUpdater.bumpPackageVersion(content, '0.0.1', 'minor');
            expect(bumpedContent).toMatchSnapshot();
            expect(bumpedContent).not.toEqual(content);
        });
        it('returns content if bumping errors', async () => {
            ...
            const npmUpdater1 = await Promise.resolve().then(() => __importStar(require('./update')));
            const { bumpedContent } = npmUpdater1.bumpPackageVersion(content, '0.0.2', true);
            expect(bumpedContent).toEqual(content);
        });
    });
});
