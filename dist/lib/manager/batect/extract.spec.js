...
function createDockerDependency(tag) {
    return {
        ...extract_1.getDep(tag),
        versioning: docker_1.id,
    };
}
function createGitDependency(repo, version) {
    return {
        depName: repo,
...
describe('lib/manager/batect/extract', () => {
    describe('extractPackageFile()', () => {
        it('returns empty array for empty configuration file', async () => {
            expect(await extract_2.extractAllPackageFiles({}, [`${fixturesDir}/empty/batect.yml`])).toEqual([]);
        });
        it('returns empty array for non-object configuration file', async () => {
            expect(await extract_2.extractAllPackageFiles({}, [`${fixturesDir}/invalid/batect.yml`])).toEqual([]);
        });
        it('returns an a package file with no dependencies for configuration file without containers or includes', async () => {
            const result = await extract_2.extractAllPackageFiles({}, [
        ...
        it('extracts all available images and bundles from a valid Batect configuration file, including dependencies in included files', async () => {
            ...
            expect(result.sort((a, b) => a.packageFile.localeCompare(b.packageFile))).toEqual([
                {
                    packageFile: `${fixturesDir}/valid/another-include.yml`,
                    deps: [
                        createDockerDependency('ubuntu:19.10'),
                        createGitDependency('https://another-include.com/my-repo.git', '4.5.6'),
                    ],
                },
                {
                    ...
                    deps: [
                        ...
                        createDockerDependency('postgres:9.6.20@sha256:166179811e4c75f8a092367afed6091208c8ecf60b111c7e49f29af45ca05e08'),
