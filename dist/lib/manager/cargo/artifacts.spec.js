...
const cargo = __importStar(require("./artifacts"));
jest.mock('fs-extra');
...
describe('.updateArtifacts()', () => {
    ...
    it('returns null if no Cargo.lock found', async () => {
        ...
        expect(await cargo.updateArtifacts({
    ...
    it('returns null if unchanged', async () => {
        fs.readFile.mockResolvedValueOnce('Current Cargo.lock');
    ...
    it('returns updated Cargo.lock', async () => {
        util_1.git.getFile.mockResolvedValueOnce('Old Cargo.lock');
        const execSnapshots = exec_util_1.mockExecAll(exec);
        fs.readFile.mockResolvedValueOnce('New Cargo.lock');
        const updatedDeps = ['dep1'];
        expect(await cargo.updateArtifacts({
            packageFileName: 'Cargo.toml',
            updatedDeps,
