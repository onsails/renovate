...
describe('lib/manager/gradle/updateGradleVersion', () => {
    ...
    it('returns the same file if dependency is null', () => {
        ...
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, null, null);
        expect(updatedGradleFile).toEqual(gradleFile);
    });
    it('returns the same file if version is not found', () => {
        const gradleFile = "runtime('mysql:mysql-connector-java:6.0.5')";
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'org.codehaus.groovy', name: 'groovy', version: '2.4.9' }, '7.0.0');
        expect(updatedGradleFile).toEqual(gradleFile);
    });
    it('returns a file updated if the version is found', () => {
        const gradleFile = "runtime (  'mysql:mysql-connector-java:6.0.5'  )";
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'mysql', name: 'mysql-connector-java', version: '6.0.5' }, '7.0.0');
        expect(updatedGradleFile).toEqual("runtime (  'mysql:mysql-connector-java:7.0.0'  )");
    });
    it('should returns a file updated with keeping an extension if the version is found', () => {
        const gradleFile = "runtime (  'com.crashlytics.sdk.android:crashlytics:2.8.0@aar'  )";
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, {
            group: 'com.crashlytics.sdk.android',
            name: 'crashlytics',
            version: '2.8.0',
        }, '2.10.1');
        expect(updatedGradleFile).toEqual("runtime (  'com.crashlytics.sdk.android:crashlytics:2.10.1@aar'  )");
    });
    it('should returns a file updated with keeping a classifier and an extension if the version is found', () => {
        ...
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'junit', name: 'junit', version: '4.0' }, '5.0');
        expect(updatedGradleFile).toEqual("runtime (  'junit:junit:5.0:javadoc@jar'  )");
    });
    it('returns an updated file if the version in single quotes defined as map is found', () => {
        const gradleFile = `compile group  : 'mysql'               ,
               ...
               version: '6.0.5'`;
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'mysql', name: 'mysql-connector-java', version: '6.0.5' }, '7.0.0');
        expect(updatedGradleFile).toEqual(`compile group  : 'mysql'               ,
               name   : 'mysql-connector-java',
               version: '7.0.0'`);
    });
    it('returns an updated file if the version in single quotes defined as map is found in a different order', () => {
        const gradleFile = `compile name : 'mysql-connector-java',
               ...
               version: '6.0.5'`;
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'mysql', name: 'mysql-connector-java', version: '6.0.5' }, '7.0.0');
        expect(updatedGradleFile).toEqual(`compile name : 'mysql-connector-java',
               group  : 'mysql',
               version: '7.0.0'`);
    });
    ...
    it('returns an updated file if the version in triple single quotes defined as map is found in a different order', () => {
        const gradleFile = `compile version: '''6.0.5''',
               ...
               name   : 'mysql-connector-java'`;
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleFile, { group: 'mysql', name: 'mysql-connector-java', version: '6.0.5' }, '7.0.0');
        expect(updatedGradleFile).toEqual(`compile version: '''7.0.0''',
               group  : 'mysql',
               name   : 'mysql-connector-java'`);
    });
    ...
    it('should replace a external groovy variable assigned to a specific dependency', () => {
        ...
        build_gradle_1.collectVersionVariables([mysqlDependency], gradleFile);
        const gradleWithVersionFile = 'String mysqlVersion = "6.0.5"';
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleWithVersionFile, mysqlDependency, '7.0.0');
        expect(updatedGradleFile).toEqual('String mysqlVersion = "7.0.0"');
    });
    it('should replace a external groovy map variable assigned to a specific dependency', () => {
        const gradleFile = 'runtime (  "mysql:mysql-connector-java:${versions.mysqlVersion}"  )'; // eslint-disable-line no-template-curly-in-string
        const mysqlDependency = {
            group: 'mysql',
            depGroup: 'mysql',
            name: 'mysql-connector-java',
        ...
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleWithVersionFile, mysqlDependency, '7.0.0');
        expect(updatedGradleFile).toEqual('ext.versions = [ mysqlVersion: "7.0.0" ]');
    });
    it('should replace a external groovy map nested variable assigned to a specific dependency', () => {
        const gradleFile = 'runtime (  "mysql:mysql-connector-java:${versions.nested.mysqlVersion}"  )'; // eslint-disable-line no-template-curly-in-string
        const mysqlDependency = {
            group: 'mysql',
            depGroup: 'mysql',
            name: 'mysql-connector-java',
        ...
        build_gradle_1.collectVersionVariables([mysqlDependency], gradleFile);
        const gradleWithVersionFile = 'ext.versions = [ nested: [ mysqlVersion: "6.0.5" ] ]';
        const updatedGradleFile = build_gradle_1.updateGradleVersion(gradleWithVersionFile, mysqlDependency, '7.0.0');
        expect(updatedGradleFile).toEqual('ext.versions = [ nested: [ mysqlVersion: "7.0.0" ] ]');
    });
    it('should replace a external property variable assigned to a specific dependency', () => {
        ...
        const updatedGradleFile = build_gradle_1.updateGradleVersion(propertyFile, mysqlDependency, '7.0.0');
    ...
    it('should replace a external property variable assigned to a specific dependency parenthesis syntax', () => {
        const gradleFile = "implementation platform(group: 'mysql', name: 'mysql-connector-java', version: mysqlVersion)"; // eslint-disable-line no-template-curly-in-string
        const mysqlDependency = {
            group: 'mysql',
            depGroup: 'mysql',
            name: 'mysql-connector-java',
...
//# sourceMappingURL=build-gradle.spec.js.map