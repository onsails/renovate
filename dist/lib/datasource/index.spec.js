...
const datasourceDocker = __importStar(require("./docker"));
const datasourceGithubTags = __importStar(require("./github-tags"));
const datasourceMaven = __importStar(require("./maven"));
const datasourceNpm = __importStar(require("./npm"));
const datasourcePackagist = __importStar(require("./packagist"));
const datasource = __importStar(require("."));
jest.mock('./docker');
jest.mock('./maven');
jest.mock('./npm');
jest.mock('./packagist');
const dockerDatasource = util_1.mocked(datasourceDocker);
const mavenDatasource = util_1.mocked(datasourceMaven);
const npmDatasource = util_1.mocked(datasourceNpm);
const packagistDatasource = util_1.mocked(datasourcePackagist);
describe('datasource/index', () => {
    ...
    it('validates dataource', () => {
        function validateDatasource(module, name) {
        ...
        const loadedDs = modules_1.loadModules(__dirname, validateDatasource);
    ...
    it('returns if digests are supported', () => {
        expect(datasource.supportsDigests({ datasource: datasourceGithubTags.id })).toBe(true);
    });
    it('returns null for no datasource', async () => {
        expect(await datasource.getPkgReleases({
            datasource: null,
            depName: 'some/dep',
        })).toBeNull();
    });
    ...
    it('returns getDigest', async () => {
        expect(await datasource.getDigest({
    ...
    it('returns null for HOST_DISABLED', async () => {
        packagistDatasource.getReleases.mockImplementationOnce(() => {
            throw new external_host_error_1.ExternalHostError(new Error(error_messages_1.HOST_DISABLED));
        });
        expect(await datasource.getPkgReleases({
            datasource: datasourcePackagist.id,
            depName: 'something',
    ...
    it('hunts registries and aborts on ExternalHostError', async () => {
        packagistDatasource.getReleases.mockImplementationOnce(() => {
            throw new external_host_error_1.ExternalHostError(new Error());
        });
        await expect(datasource.getPkgReleases({
            datasource: datasourcePackagist.id,
            depName: 'something',
            registryUrls: ['https://reg1.com', 'https://reg2.io'],
