...
const docker = __importStar(require("."));
...
describe(util_1.getName(__filename), () => {
    ...
    describe('getRegistryRepository', () => {
        it('handles local registries', () => {
            const res = docker.getRegistryRepository('registry:5000/org/package', 'https://index.docker.io');
            expect(res).toMatchSnapshot();
        });
        it('supports registryUrls', () => {
            const res = docker.getRegistryRepository('my.local.registry/prefix/image', 'https://my.local.registry/prefix');
            expect(res).toMatchSnapshot();
        });
    });
    describe('getDigest', () => {
        it('returns null if no token', async () => {
            httpMock
                ...
                .reply(401);
            const res = await __1.getDigest({ datasource: 'docker', depName: 'some-dep' }, 'some-new-value');
            expect(res).toBeNull();
        ...
        it('returns digest', async () => {
            ...
            httpMock
                .scope(authUrl)
                .get('/token?service=registry.docker.io&scope=repository:library/some-dep:pull')
                .reply(200, { token: 'some-token' });
            hostRules.find.mockReturnValue({});
            const res = await __1.getDigest({
                datasource: 'docker',
                depName: 'some-dep',
            });
        ...
        it('falls back to body for digest', async () => {
          ...
          "signatures": [
            ...
            httpMock
                ...
                .reply(200, { token: 'some-token' });
            const res = await __1.getDigest({ datasource: 'docker', depName: 'some-dep' }, 'some-new-value');
            expect(res).toBe('sha256:b3d6068234f3a18ebeedd2dab81e67b6a192e81192a099df4112ecfc7c3be84f');
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        ...
        it('supports basic authentication', async () => {
            httpMock
                ...
                .reply(200, '', { 'docker-content-digest': 'some-digest' });
            const res = await __1.getDigest({ datasource: 'docker', depName: 'some-dep' }, 'some-tag');
            const trace = httpMock.getTrace();
            expect(res).toBe('some-digest');
            expect(trace[1].headers.authorization).toBe('Basic c29tZS11c2VybmFtZTpzb21lLXBhc3N3b3Jk');
        ...
        it('supports ECR authentication', async () => {
            ...
            aws_sdk_mock_1.default.mock('ECR', 'getAuthorizationToken', (params, callback) => {
                callback(null, {
            ...
            expect(trace).toMatchSnapshot();
            aws_sdk_mock_1.default.restore('ECR');
        });
        ...
        it('supports scoped names', async () => {
            ...
            httpMock
                .scope(authUrl)
                .get('/token?service=registry.docker.io&scope=repository:library/some-other-dep:pull')
                .reply(200, { access_token: 'some-token' });
            const res = await __1.getDigest({ datasource: 'docker', depName: 'some-other-dep' }, '8.0.0-alpine');
            expect(res).toBe('some-digest');
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('should throw error for 429', async () => {
            httpMock.scope(baseUrl).get('/').replyWithError({ statusCode: 429 });
            await expect(__1.getDigest({ datasource: 'docker', depName: 'some-dep' }, 'latest')).rejects.toThrow(error_messages_1.EXTERNAL_HOST_ERROR);
        });
    ...
    describe('getReleases', () => {
        ...
        it('adds library/ prefix for Docker Hub (implicit)', async () => {
            ...
            httpMock
                .scope(authUrl)
                .get('/token?service=registry.docker.io&scope=repository:library/node:pull')
                .reply(200, { token: 'some-token ' });
            const res = await __1.getPkgReleases({
                datasource: docker.id,
                depName: 'node',
        ...
        it('adds no library/ prefix for other registries', async () => {
            ...
            httpMock
                ...
                .get('/token?service=k8s.gcr.io&scope=repository:kubernetes-dashboard-amd64:pull')
                .reply(200, { token: 'some-token ' })
        ...
        it('supports redirect', async () => {
            httpMock
                ...
                .get('/node/blobs/some-config-digest')
                .reply(302, undefined, {
                location: 'https://abc.s3.amazon.com/some-config-digest?X-Amz-Algorithm=xxxx',
            });
            httpMock
