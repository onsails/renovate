...
const _01_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/01.json"));
const _02_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/02.json"));
const coffeelint_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/coffeelint.json"));
const next_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/next.json"));
const typescript_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/typescript.json"));
const vue_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/vue.json"));
const webpack_json_1 = __importDefault(require("../../../../config/npm/__fixtures__/webpack.json"));
...
const git_submodules_1 = require("../../../../datasource/git-submodules");
...
const poetry_1 = require("../../../../versioning/poetry");
const lookup = __importStar(require("."));
jest.mock('../../../../datasource/docker');
jest.mock('../../../../datasource/git-submodules');
jest.mock('../../../../datasource/github-releases');
_01_json_1.default.latestVersion = '1.4.1';
const docker = util_1.mocked(datasourceDocker);
docker.defaultRegistryUrls = ['https://index.docker.io'];
const gitSubmodules = util_1.mocked(datasourceGitSubmodules);
const githubReleases = util_1.mocked(datasourceGithubReleases);
Object.assign(githubReleases, { defaultRegistryUrls: ['https://github.com'] });
let config;
describe('workers/repository/process/lookup', () => {
    beforeEach(() => {
        // TODO: fix types
        config = util_1.partial(util_1.getConfig());
        config.manager = 'npm';
        config.versioning = npm_2.id;
    ...
    describe('.lookupUpdates()', () => {
        it('returns rollback for pinned version', async () => {
            ...
            config.depName = 'q';
            config.datasource = npm_1.id;
            config.rollbackPrs = true;
            nock_1.default('https://registry.npmjs.org').get('/q').reply(200, _01_json_1.default);
        ...
        it('falls back to semver syntax allowedVersions', async () => {
            ...
            config.allowedVersions = '<1';
            config.depName = 'q';
            config.versioning = docker_2.id; // this doesn't make sense but works for this test
            config.datasource = npm_1.id; // this doesn't make sense but works for this test
            nock_1.default('https://registry.npmjs.org').get('/q').reply(200, _01_json_1.default);
            expect((await lookup.lookupUpdates(config)).updates).toHaveLength(1);
        });
        it('falls back to pep440 syntax allowedVersions', async () => {
            ...
            config.depName = 'q';
            config.versioning = poetry_1.id; // this doesn't make sense but works for this test
            config.datasource = npm_1.id; // this doesn't make sense but works for this test
            nock_1.default('https://registry.npmjs.org').get('/q').reply(200, _01_json_1.default);
            expect((await lookup.lookupUpdates(config)).updates).toHaveLength(1);
        });
        it('skips invalid allowedVersions', async () => {
            ...
            config.datasource = npm_1.id;
            nock_1.default('https://registry.npmjs.org').get('/q').reply(200, _01_json_1.default);
            await expect(lookup.lookupUpdates(config)).rejects.toThrow(Error(error_messages_1.CONFIG_VALIDATION));
        });
        ...
        it('widens major ranged versions if configured', async () => {
            ...
            nock_1.default('https://registry.npmjs.org')
                .get('/webpack')
                .reply(200, webpack_json_1.default);
            expect((await lookup.lookupUpdates(config)).updates).toMatchSnapshot();
        });
        ...
        it('should ignore unstable versions if the current version is stable', async () => {
            ...
            config.depName = 'vue';
            config.datasource = npm_1.id;
            nock_1.default('https://registry.npmjs.org').get('/vue').reply(200, vue_json_1.default);
            expect((await lookup.lookupUpdates(config)).updates).toHaveLength(0);
        });
        ...
        it('should allow unstable versions if the current version is unstable', async () => {
            ...
            nock_1.default('https://registry.npmjs.org')
                .get('/typescript')
                .reply(200, typescript_json_1.default);
            const res = await lookup.lookupUpdates(config);
            expect(res.updates).toMatchSnapshot();
            expect(res.updates).toHaveLength(1);
        ...
        it('should downgrade from missing versions', async () => {
            ...
            nock_1.default('https://registry.npmjs.org')
                .get('/coffeelint')
                .reply(200, coffeelint_json_1.default);
            const res = await lookup.lookupUpdates(config);
            expect(res.updates).toHaveLength(1);
            expect(res.updates[0]).toMatchSnapshot();
        });
        ...
        it('does not jump  major unstable', async () => {
            ...
            config.depName = 'next';
            config.datasource = npm_1.id;
            nock_1.default('https://registry.npmjs.org').get('/next').reply(200, next_json_1.default);
            const res = await lookup.lookupUpdates(config);
            expect(res.updates).toHaveLength(0);
        });
        ...
        it('handles github 404', async () => {
            config.depName = 'foo';
            config.datasource = github_tags_1.id;
            config.packageFile = 'package.json';
            config.currentValue = '1.0.0';
            nock_1.default('https://pypi.org').get('/pypi/foo/json').reply(404);
            expect((await lookup.lookupUpdates(config)).updates).toMatchSnapshot();
        });
        it('handles pypi 404', async () => {
            config.depName = 'foo';
            config.datasource = pypi_1.id;
        ...
        it('handles packagist', async () => {
            ...
            config.datasource = packagist_1.id;
            config.packageFile = 'composer.json';
            config.currentValue = '1.0.0';
            config.registryUrls = ['https://packagist.org'];
            nock_1.default('https://packagist.org').get('/packages/foo/bar.json').reply(404);
            expect((await lookup.lookupUpdates(config)).updates).toMatchSnapshot();
        });
        ...
        it('ignores deprecated', async () => {
            ...
            config.depName = 'q2';
            config.datasource = npm_1.id;
            const returnJson = JSON.parse(JSON.stringify(_01_json_1.default));
            returnJson.name = 'q2';
            returnJson.versions['1.4.1'].deprecated = 'true';
            nock_1.default('https://registry.npmjs.org').get('/q2').reply(200, returnJson);
            const res = await lookup.lookupUpdates(config);
            expect(res).toMatchSnapshot();
        ...
        it('is deprecated', async () => {
            ...
            config.datasource = npm_1.id;
            const returnJson = {
                ...JSON.parse(JSON.stringify(_01_json_1.default)),
                name: 'q3',
                deprecated: true,
                repository: { url: null, directory: 'test' },
            };
            nock_1.default('https://registry.npmjs.org').get('/q3').reply(200, returnJson);
            const res = await lookup.lookupUpdates(config);
            expect(res).toMatchSnapshot();
        ...
        it('handles git submodule update', async () => {
            ...
            config.versioning = git_1.id;
            config.datasource = git_submodules_1.id;
