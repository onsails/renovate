...
const presets = __importStar(require("../../../../config/presets/local"));
const util_2 = require("../../../../config/presets/util");
const config_1 = require("./config");
jest.mock('../../../../config/presets/local');
const mockedPresets = presets;
describe('workers/repository/onboarding/branch', () => {
    ...
    describe('getOnboardingConfig', () => {
        it('handles finding an organization preset', async () => {
            ...
            onboardingConfig = await config_1.getOnboardingConfig(config);
        ...
        it('handles finding an organization dot platform preset', async () => {
            mockedPresets.getPreset.mockRejectedValueOnce(new Error(util_2.PRESET_DEP_NOT_FOUND));
            mockedPresets.getPreset.mockResolvedValueOnce({ enabled: true });
            onboardingConfig = await config_1.getOnboardingConfig(config);
            expect(mockedPresets.getPreset).toHaveBeenCalledTimes(2);
            expect(JSON.parse(onboardingConfig).extends[0]).toEqual('local>some/.github:renovate-config');
        });
        it('handles not finding an organization preset', async () => {
            mockedPresets.getPreset.mockRejectedValue(new Error(util_2.PRESET_DEP_NOT_FOUND));
            onboardingConfig = await config_1.getOnboardingConfig(config);
            expect(mockedPresets.getPreset).toHaveBeenCalledTimes(2);
            expect(JSON.parse(onboardingConfig)).toEqual(config.onboardingConfig);
        });
        it('ignores an unknown error', async () => {
            mockedPresets.getPreset.mockRejectedValue(new Error('unknown error for test'));
            onboardingConfig = await config_1.getOnboardingConfig(config);
            expect(mockedPresets.getPreset).toHaveBeenCalledTimes(2);
            expect(JSON.parse(onboardingConfig)).toEqual(config.onboardingConfig);
        });
        it('ignores unsupported platform', async () => {
            mockedPresets.getPreset.mockRejectedValue(new Error(`Unsupported platform 'dummy' for local preset.`));
            onboardingConfig = await config_1.getOnboardingConfig(config);
            expect(mockedPresets.getPreset).toHaveBeenCalledTimes(2);
            expect(JSON.parse(onboardingConfig)).toEqual(config.onboardingConfig);
        });
