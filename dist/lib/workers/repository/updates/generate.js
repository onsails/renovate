...
function generateBranchConfig(branchUpgrades) {
    ...
    for (const branchUpgrade of branchUpgrades) {
        ...
        if (upgrade.semanticCommits === 'enabled' && !upgrade.commitMessagePrefix) {
            ...
            upgrade.commitMessagePrefix = commit_message_1.formatCommitMessagePrefix(semanticPrefix);
            upgrade.toLowerCase =
    ...
    config.constraints = Object.assign({}, ...config.upgrades.map((upgrade) => upgrade.constraints));
