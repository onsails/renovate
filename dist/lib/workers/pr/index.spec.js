...
const _changelogHelper = __importStar(require("./changelog"));
const changelog_1 = require("./changelog");
const codeOwners = __importStar(require("./code-owners"));
const prWorker = __importStar(require("."));
const codeOwnersMock = util_1.mocked(codeOwners);
const changelogHelper = util_1.mocked(_changelogHelper);
const gitlabChangelogHelper = util_1.mocked(_changelogHelper);
const platform = util_1.mocked(platform_1.platform);
...
describe('workers/pr', () => {
    describe('checkAutoMerge(pr, config)', () => {
        ...
        it('should automerge if enabled and pr is mergeable', async () => {
            ...
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.green);
        ...
        it('should not automerge if enabled and pr is mergeable but branch status is not success', async () => {
            config.automerge = true;
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.yellow);
            await prWorker.checkAutoMerge(pr, config);
            expect(platform.mergePr).toHaveBeenCalledTimes(0);
        });
    ...
    describe('ensurePr', () => {
        ...
        it('should return null if check fails', async () => {
            ...
            expect(prResult).toEqual(common_1.PrResult.Error);
            expect(pr).toBeUndefined();
        });
        it('should return null if waiting for success', async () => {
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.red);
            config.prCreation = 'status-success';
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.AwaitingGreenBranch);
            expect(pr).toBeUndefined();
        });
        it('should return needs-approval if prCreation set to approval', async () => {
            ...
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.AwaitingApproval);
            expect(pr).toBeUndefined();
        });
        it('should create PR if success for gitlab deps', async () => {
            ...
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Created);
        ...
        it('should create PR if success', async () => {
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.green);
            config.logJSON = await changelog_1.getChangeLogJSON(config);
            config.prCreation = 'status-success';
            config.automerge = true;
            config.schedule = ['before 5am'];
            const { prResult, pr } = await prWorker.ensurePr(config);
        ...
        it('should not create PR if limit is reached', async () => {
            ...
            expect(prResult).toEqual(common_1.PrResult.LimitReached);
        ...
        it('should create group PR', async () => {
            ...
            for (const upgrade of config.upgrades) {
                upgrade.logJSON = await changelog_1.getChangeLogJSON(upgrade);
            }
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Created);
            expect(pr).toMatchObject({ displayNumber: 'New Pull Request' });
        ...
        it('should add note about Pin', async () => {
            ...
            expect(pr).toMatchObject({ displayNumber: 'New Pull Request' });
            expect(platform.createPr.mock.calls[0]).toMatchSnapshot();
            expect(platform.createPr.mock.calls[0][0].prBody).toContain('this Pin PR');
        });
        ...
        it('should return null if waiting for not pending', async () => {
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.yellow);
            util_1.git.getBranchLastCommitTime.mockImplementationOnce(() => Promise.resolve(new Date()));
            config.prCreation = 'not-pending';
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.AwaitingNotPending);
            expect(pr).toBeUndefined();
        });
        ...
        it('should create PR if pending timeout hit', async () => {
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.yellow);
            util_1.git.getBranchLastCommitTime.mockImplementationOnce(() => Promise.resolve(new Date('2017-01-01')));
            config.prCreation = 'not-pending';
            config.stabilityStatus = types_1.BranchStatus.yellow;
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Created);
        ...
        it('should return unmodified existing PR', async () => {
            ...
            config.logJSON = await changelog_1.getChangeLogJSON(config);
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.NotUpdated);
        ...
        it('should return unmodified existing PR if only whitespace changes', async () => {
            const modifiedPr = JSON.parse(JSON.stringify(existingPr).replace(' ', '  ').replace('\n', '\r\n'));
        ...
        it('should return modified existing PR title', async () => {
            ...
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Updated);
            expect(pr).toMatchSnapshot();
        });
        ...
        it('should return no PR if branch automerging not failed', async () => {
            ...
            config.automergeType = 'branch';
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.yellow);
            util_1.git.getBranchLastCommitTime.mockResolvedValueOnce(new Date());
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.BlockedByBranchAutomerge);
            expect(pr).toBeUndefined();
        });
        it('should return PR if branch automerging taking too long', async () => {
            ...
            config.automergeType = 'branch';
            platform.getBranchStatus.mockResolvedValueOnce(types_1.BranchStatus.yellow);
            util_1.git.getBranchLastCommitTime.mockResolvedValueOnce(new Date('2018-01-01'));
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Created);
        ...
        it('should create PR if waiting for not pending but artifactErrors', async () => {
            ...
            config.platform = platforms_1.PLATFORM_TYPE_GITLAB;
            const { prResult, pr } = await prWorker.ensurePr(config);
            expect(prResult).toEqual(common_1.PrResult.Created);
            expect(pr).toMatchObject({ displayNumber: 'New Pull Request' });
        });
