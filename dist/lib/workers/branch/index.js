...
async function processBranch(branchConfig) {
    ...
    await git_1.checkoutBranch(config.baseBranch);
    const branchExists = git_1.branchExists(config.branchName);
    ...
    try {
        ...
        if (existingPr && !dependencyDashboardCheck) {
            ...
            if (existingPr.state === types_1.PrState.Closed) {
                ...
                if (!config.suppressNotifications.includes('prIgnoreNotification')) {
                    if (config.dryRun) {
                        logger_1.logger.info(`DRY-RUN: Would ensure closed PR comment in PR #${existingPr.number}`);
                    }
                    else {
                ...
                if (branchExists) {
                    if (config.dryRun) {
                        ...
                        await git_1.deleteBranch(config.branchName);
                    }
                }
            }
            else if (existingPr.state === types_1.PrState.Merged) {
                logger_1.logger.debug({ pr: existingPr.number }, 'Merged PR is blocking this branch');
            }
            return common_1.ProcessBranchResult.AlreadyExisted;
        }
        // istanbul ignore if
        if (!branchExists && config.dependencyDashboardApproval) {
            if (dependencyDashboardCheck) {
                ...
                return common_1.ProcessBranchResult.NeedsApproval;
            }
        }
        if (!branchExists &&
            limits_1.isLimitReached(limits_1.Limit.Branches) &&
            !dependencyDashboardCheck &&
            !config.vulnerabilityAlert) {
            logger_1.logger.debug('Reached branch limit - skipping branch creation');
            return common_1.ProcessBranchResult.BranchLimitReached;
        }
        if (limits_1.isLimitReached(limits_1.Limit.Commits) &&
            !dependencyDashboardCheck &&
            !config.vulnerabilityAlert) {
            logger_1.logger.debug('Reached commits limit - skipping branch');
            return common_1.ProcessBranchResult.CommitLimitReached;
        }
        if (branchExists) {
                    ...
                    if (dependencyDashboardCheck || config.rebaseRequested) {
                        ...
                        if (newBody !== branchPr.body) {
                            ...
                            await platform_1.platform.updatePr({
                                ...
                                prBody: newBody,
                                platformOptions: pr_1.getPlatformPrOptions(config),
                            });
                        }
                        return common_1.ProcessBranchResult.PrEdited;
                    }
                }
            }
            else if (branchIsModified) {
                logger_1.logger.debug('Branch has been edited');
                return common_1.ProcessBranchResult.PrEdited;
            }
        }
        // Check schedule
        ...
        if (!config.isScheduledNow && !dependencyDashboardCheck) {
            if (!branchExists) {
                ...
                return common_1.ProcessBranchResult.NotScheduled;
            }
        ...
        if (config.upgrades.some((upgrade) => upgrade.stabilityDays && upgrade.releaseTimestamp)) {
            ...
            if (!dependencyDashboardCheck &&
                ...
                return common_1.ProcessBranchResult.Pending;
            }
        }
        // istanbul ignore if
        ...
        (((_d = config.updatedPackageFiles) === null || _d === void 0 ? void 0 : _d.length) > 0 ||
            ...
            for (const upgrade of config.upgrades) {
                logger_1.addMeta({ dep: upgrade.depName });
                ...
                if (is_1.default.nonEmptyArray(commands)) {
                    ...
                    for (const cmd of commands) {
                        if (!config.allowedPostUpgradeCommands.some((pattern) => regex_1.regEx(pattern).test(cmd))) {
                            ...
                            const execResult = await exec_1.exec(compiledCmd, {
                                cwd: config.localDir,
                            });
                    ...
                    for (const relativePath of status.modified.concat(status.not_added)) {
                        for (const pattern of fileFilters) {
                            if (minimatch_1.default(relativePath, pattern)) {
                                ...
                                const existingUpdatedArtifacts = config.updatedArtifacts.find((ua) => ua.name === relativePath);
                                ...
                                config.updatedArtifacts = config.updatedArtifacts.filter((ua) => ua.name !== '|delete|' || ua.contents !== relativePath);
                            }
                        }
                    }
                    for (const relativePath of status.deleted || []) {
                        for (const pattern of fileFilters) {
                            if (minimatch_1.default(relativePath, pattern)) {
                                ...
                                config.updatedArtifacts = config.updatedArtifacts.filter((ua) => ua.name !== relativePath);
                            }
                        }
                    }
                }
        ...
        if (branchExists || !config.requiredStatusChecks) {
            ...
            if (mergeStatus === 'automerged') {
                ...
                return common_1.ProcessBranchResult.Automerged;
            }
        ...
        if (err.message === error_messages_1.WORKER_FILE_UPDATE_FAILED) {
            ...
            return common_1.ProcessBranchResult.Error;
        }
            ...
            logger_1.logger.error({ err }, `Error updating branch: ${String(err.message)}`);
        }
    ...
    try {
        ...
        if (pr) {
            ...
            if ((_m = config.artifactErrors) === null || _m === void 0 ? void 0 : _m.length) {
                ...
                content = platform_1.platform.getPrBody(content);
                ...
                if ((_o = config.updatedArtifacts) === null || _o === void 0 ? void 0 : _o.length) {
                    // istanbul ignore if
                    if (config.dryRun) {
    ...
    if (!branchExists) {
        return common_1.ProcessBranchResult.PrCreated;
    }
    return common_1.ProcessBranchResult.Done;
}
