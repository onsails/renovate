...
const html_1 = require("../../manager/html");
const auto_replace_1 = require("./auto-replace");
const sampleHtml = fs_1.readFileSync(upath_1.resolve(__dirname, `../../manager/html/__fixtures__/sample.html`), 'utf8');
jest.mock('../../util/fs');
describe('workers/branch/auto-replace', () => {
    describe('doAutoReplace', () => {
        ...
        beforeEach(() => {
            upgrade = {
                ...JSON.parse(JSON.stringify(util_1.defaultConfig)),
                manager: 'html',
            };
            reuseExistingBranch = false;
        });
        it('rebases if the deps list has changed', async () => {
            upgrade.baseDeps = html_1.extractPackageFile(sampleHtml).deps;
            reuseExistingBranch = true;
            const res = await auto_replace_1.doAutoReplace(upgrade, 'existing content', reuseExistingBranch);
            expect(res).toBeNull();
        });
        it('rebases if the deps to update has changed', async () => {
            ...
            reuseExistingBranch = true;
            const res = await auto_replace_1.doAutoReplace(upgrade, sampleHtml, reuseExistingBranch);
            expect(res).toBeNull();
        });
        it('updates version only', async () => {
            const script = '<script src="https://cdnjs.cloudflare.com/ajax/libs/reactstrap/7.1.0/reactstrap.min.js">';
            const src = `     ${script}   `;
            upgrade.baseDeps = html_1.extractPackageFile(src).deps;
            ...
            upgrade.depIndex = 0;
            const res = await auto_replace_1.doAutoReplace(upgrade, src, reuseExistingBranch);
        ...
        it('handles already updated', async () => {
            ...
            const res = await auto_replace_1.doAutoReplace(upgrade, srcAlreadyUpdated, reuseExistingBranch);
            expect(res).toMatchSnapshot();
        });
        it('returns existing content if replaceString mismatch', async () => {
            ...
            upgrade.depIndex = 0;
            upgrade.replaceString = script;
            const res = await auto_replace_1.doAutoReplace(upgrade, 'wrong source', reuseExistingBranch);
            expect(res).toEqual('wrong source');
        });
        it('updates version and integrity', async () => {
            const script = '<script src="https://cdnjs.cloudflare.com/ajax/libs/KaTeX/0.10.0/katex.min.js" integrity="sha384-K3vbOmF2BtaVai+Qk37uypf7VrgBubhQreNQe9aGsz9lB63dIFiQVlJbr92dw2Lx" crossorigin="anonymous">';
            const src = `     ${script}   `;
            upgrade.baseDeps = html_1.extractPackageFile(src).deps;
        ...
        it('updates with autoReplaceNewString', async () => {
            const dockerfile = 'FROM node:8.11.3-alpine@sha256:d743b4141b02fcfb8beb68f92b4cd164f60ee457bf2d053f36785bf86de16b0d AS node';
            ...
            const res = await auto_replace_1.doAutoReplace(upgrade, dockerfile, reuseExistingBranch);
            expect(res).toMatchSnapshot();
        });
        it('fails with oldversion in depname', async () => {
            const yml = 'image: "1111111111.dkr.ecr.us-east-1.amazonaws.com/my-repository:1"\n\n';
            ...
            const res = auto_replace_1.doAutoReplace(upgrade, yml, reuseExistingBranch);
            await expect(res).rejects.toThrow(error_messages_1.WORKER_FILE_UPDATE_FAILED);
        });
    });
});
//# sourceMappingURL=auto-replace.spec.js.map