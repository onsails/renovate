...
const git = __importStar(require("."));
describe('platform/git', () => {
    ...
    let defaultBranch;
    beforeAll(async () => {
        base = await tmp_promise_1.default.dir({ unsafeCleanup: true });
        const repo = simple_git_1.default(base.path);
        await repo.init();
        ...
        await fs_extra_1.default.writeFile(base.path + '/past_file', 'past');
        ...
        await fs_extra_1.default.writeFile(base.path + '/master_file', defaultBranch);
        await fs_extra_1.default.writeFile(base.path + '/file_to_delete', 'bye');
        ...
        await fs_extra_1.default.writeFile(base.path + '/future_file', 'future');
        ...
        await fs_extra_1.default.writeFile(base.path + '/base_file', 'base');
        await repo.add(['base_file']);
        await repo.commit('base message');
        await fs_extra_1.default.writeFile(base.path + '/modified_file', 'modified');
        await repo.add(['modified_file']);
        await repo.commit('modification');
        await repo.checkoutBranch('renovate/custom_author', defaultBranch);
        await fs_extra_1.default.writeFile(base.path + '/custom_file', 'custom');
    ...
    let tmpDir;
    beforeEach(async () => {
        origin = await tmp_promise_1.default.dir({ unsafeCleanup: true });
        const repo = simple_git_1.default(origin.path);
    ...
    describe('getFileList()', () => {
        ...
        it('should exclude submodules', async () => {
            ...
            await git.syncGit();
            expect(await fs_extra_1.default.exists(tmpDir.path + '/.gitmodules')).toBeTruthy();
            expect(await git.getFileList()).toMatchSnapshot();
    ...
    describe('getBranchFiles(branchName)', () => {
        it('detects changed files compared to current base branch', async () => {
            ...
            const branchFiles = await git.getBranchFiles('renovate/branch_with_changes');
    ...
    describe('mergeBranch(branchName)', () => {
        it('should perform a branch merge', async () => {
            ...
            const merged = await simple_git_1.default(origin.path).branch([
                '--verbose',
                '--merged',
                defaultBranch,
            ]);
    ...
    describe('deleteBranch(branchName)', () => {
        it('should send delete', async () => {
            ...
            const branches = await simple_git_1.default(origin.path).branch({});
    ...
    describe('commitFiles({branchName, files, message})', () => {
        ...
        it('does not push when no diff', async () => {
            ...
            const local = simple_git_1.default(tmpDir.path);
    ...
    describe('initRepo())', () => {
        it('should fetch latest', async () => {
            ...
            await fs_extra_1.default.writeFile(base.path + '/test', 'lorem ipsum');
            await repo.add(['test']);
        ...
        it('should fail clone ssh submodule', async () => {
            const repo = simple_git_1.default(base.path);
            await fs_extra_1.default.writeFile(base.path + '/.gitmodules', '[submodule "test"]\npath=test\nurl=ssh://0.0.0.0');
        ...
        it('should use extra clone configuration', async () => {
            await fs_extra_1.default.emptyDir(tmpDir.path);
            await git.initRepo({
                localDir: tmpDir.path,
                url: origin.path,
                extraCloneOpts: {
            ...
            await git.syncGit();
            const repo = simple_git_1.default(tmpDir.path);
