...
describe('platform/gitea', () => {
    ...
    const mockRepos = [
        util_1.partial({ full_name: 'a/b' }),
        util_1.partial({ full_name: 'c/d' }),
    ];
    const mockPRs = [
        util_1.partial({
            number: 1,
            ...
            head: {
                ...
                sha: 'some-head-sha',
                repo: util_1.partial({ full_name: mockRepo.full_name }),
            },
        }),
        util_1.partial({
            number: 2,
    ...
    beforeEach(async () => {
        ...
        jest.mock('../../util/git');
        jest.mock('../../logger');
        gitea = await Promise.resolve().then(() => __importStar(require('.')));
        helper = (await Promise.resolve().then(() => __importStar(require('./gitea-helper'))));
        logger = (await Promise.resolve().then(() => __importStar(require('../../logger')))).logger;
        ...
        gitea_1.setBaseUrl('https://gitea.renovatebot.com/api/v1');
    });
    function initFakeRepo(repo, config) {
    ...
    describe('initPlatform()', () => {
        ...
        it('should support default endpoint', async () => {
            ...
            expect(await gitea.initPlatform({ token: 'some-token' })).toMatchSnapshot();
        });
        it('should support custom endpoint', async () => {
            helper.getCurrentUser.mockResolvedValueOnce(mockUser);
            expect(await gitea.initPlatform({
                token: 'some-token',
    ...
    describe('initRepo', () => {
        ...
        it('should abort when repo is archived', async () => {
            await expect(initFakeRepo({ archived: true })).rejects.toThrow(error_messages_1.REPOSITORY_ARCHIVED);
        });
        it('should abort when repo is mirrored', async () => {
            await expect(initFakeRepo({ mirror: true })).rejects.toThrow(error_messages_1.REPOSITORY_MIRRORED);
        });
        it('should abort when repo is empty', async () => {
            await expect(initFakeRepo({ empty: true })).rejects.toThrow(error_messages_1.REPOSITORY_EMPTY);
        });
        it('should abort when repo has insufficient permissions', async () => {
            await expect(initFakeRepo({
                permissions: {
                    pull: false,
                    push: false,
                    admin: false,
                },
            })).rejects.toThrow(error_messages_1.REPOSITORY_ACCESS_FORBIDDEN);
        });
        it('should abort when repo has no available merge methods', async () => {
            await expect(initFakeRepo({ allow_rebase: false })).rejects.toThrow(error_messages_1.REPOSITORY_BLOCKED);
        });
        it('should fall back to merge method "rebase-merge"', async () => {
            expect(await initFakeRepo({ allow_rebase: false, allow_rebase_explicit: true })).toMatchSnapshot();
        });
        it('should fall back to merge method "squash"', async () => {
            expect(await initFakeRepo({ allow_rebase: false, allow_squash_merge: true })).toMatchSnapshot();
        });
        it('should fall back to merge method "merge"', async () => {
            expect(await initFakeRepo({
                allow_rebase: false,
    ...
    describe('setBranchStatus', () => {
        const setBranchStatus = async (bsc) => {
            await initFakeRepo();
        ...
        it('should create a new commit status', async () => {
            ...
            expect(helper.createCommitStatus).toHaveBeenCalledWith(mockRepo.full_name, mockCommitHash, {
                state: 'success',
                context: 'some-context',
                description: 'some-description',
            });
    ...
    describe('getBranchStatus', () => {
        const getBranchStatus = async (state) => {
            await initFakeRepo();
            helper.getCombinedCommitStatus.mockResolvedValueOnce(util_1.partial({
                worstStatus: state,
            }));
            return gitea.getBranchStatus('some-branch', []);
        };
        it('should return success if requiredStatusChecks null', async () => {
            expect(await gitea.getBranchStatus('some-branch', null)).toEqual(types_1.BranchStatus.green);
        });
        it('should return failed if unsupported requiredStatusChecks', async () => {
            expect(await gitea.getBranchStatus('some-branch', ['foo'])).toEqual(types_1.BranchStatus.red);
        });
        it('should return yellow for unknown result', async () => {
            expect(await getBranchStatus('unknown')).toEqual(types_1.BranchStatus.yellow);
        });
        it('should return pending state for pending result', async () => {
            expect(await getBranchStatus('pending')).toEqual(types_1.BranchStatus.yellow);
        });
        it('should return success state for success result', async () => {
            expect(await getBranchStatus('success')).toEqual(types_1.BranchStatus.green);
        });
        it('should return null for all other results', async () => {
            expect(await getBranchStatus('invalid')).toEqual(types_1.BranchStatus.yellow);
        });
        it('should abort when branch status returns 404', async () => {
            helper.getCombinedCommitStatus.mockRejectedValueOnce({ statusCode: 404 });
            await expect(gitea.getBranchStatus('some-branch', [])).rejects.toThrow(error_messages_1.REPOSITORY_CHANGED);
        });
        it('should propagate any other errors', async () => {
            helper.getCombinedCommitStatus.mockRejectedValueOnce(new Error('getCombinedCommitStatus()'));
            await expect(gitea.getBranchStatus('some-branch', [])).rejects.toThrow('getCombinedCommitStatus()');
        });
    });
    describe('getBranchStatusCheck', () => {
        it('should return null with no results', async () => {
            ...
            expect(await gitea.getBranchStatusCheck('some-branch', 'some-context')).toBeNull();
        });
        it('should return null with no matching results', async () => {
            helper.getCombinedCommitStatus.mockResolvedValueOnce(util_1.partial({
                statuses: [util_1.partial({ context: 'other-context' })],
            }));
            expect(await gitea.getBranchStatusCheck('some-branch', 'some-context')).toBeNull();
        });
        it('should return yellow with unknown status', async () => {
            ...
            expect(await gitea.getBranchStatusCheck('some-branch', 'some-context')).toEqual(types_1.BranchStatus.yellow);
        });
        it('should return green of matching result', async () => {
            ...
            expect(await gitea.getBranchStatusCheck('some-branch', 'some-context')).toEqual(types_1.BranchStatus.green);
        });
    });
    describe('getPrList', () => {
    ...
    describe('getPr', () => {
        it('should return enriched pull request which exists if open', async () => {
            ...
            helper.searchPRs.mockResolvedValueOnce(mockPRs);
            helper.getBranch.mockResolvedValueOnce(util_1.partial({
                commit: {
                    id: mockCommitHash,
                    author: util_1.partial({ email: global.gitAuthor.email }),
                },
            }));
            await initFakeRepo();
    ...
    describe('createPr', () => {
        ...
        it('should attempt to resolve 409 conflict error (w/ update)', async () => {
            ...
            expect(helper.updatePR).toHaveBeenCalledWith(mockRepo.full_name, mockNewPR.number, { title: 'new-title', body: 'new-body' });
        });
        it('should abort when response for created pull request is invalid', async () => {
            helper.createPR.mockResolvedValueOnce(util_1.partial({}));
            await initFakeRepo();
            await expect(gitea.createPr({
                sourceBranch: mockNewPR.head.label,
                targetBranch: 'master',
                prTitle: mockNewPR.title,
                prBody: mockNewPR.body,
    ...
    describe('mergePr', () => {
        it('should return true when merging succeeds', async () => {
            ...
            expect(helper.mergePR).toHaveBeenCalledWith(mockRepo.full_name, 1, 'rebase');
        });
    ...
    describe('findIssue', () => {
        it('should return existing open issue', async () => {
            ...
            await initFakeRepo();
            expect(await gitea.findIssue(mockIssue.title)).toHaveProperty('number', mockIssue.number);
        });
    ...
    describe('ensureIssue', () => {
        it('should create issue if not found', async () => {
            ...
            helper.searchIssues.mockResolvedValueOnce(mockIssues);
            helper.createIssue.mockResolvedValueOnce(util_1.partial({ number: 42 }));
            await initFakeRepo();
        ...
        it('should not reopen closed issue by default', async () => {
            ...
            expect(helper.updateIssue).toHaveBeenCalledWith(mockRepo.full_name, closedIssue.number, {
                body: closedIssue.body,
        ...
        it('should close all open duplicate issues except first one when updating', async () => {
            ...
            for (const issue of duplicates) {
                if (issue.number !== firstDuplicate.number) {
                    ...
                    expect(helper.closeIssue).toHaveBeenCalledWith(mockRepo.full_name, issue.number);
                }
            }
            expect(helper.updateIssue).not.toHaveBeenCalled();
        });
    ...
    describe('ensureIssueClosing', () => {
        it('should close issues with matching title', async () => {
            ...
            expect(helper.closeIssue).toHaveBeenCalledWith(mockRepo.full_name, mockIssue.number);
        });
    });
    describe('deleteLabel', () => {
        it('should delete a label which exists', async () => {
            ...
            expect(helper.unassignLabel).toHaveBeenCalledWith(mockRepo.full_name, 42, mockLabel.id);
        });
    ...
    describe('ensureComment', () => {
        it('should add comment with topic if not found', async () => {
            ...
            helper.createComment.mockResolvedValueOnce(util_1.partial({ id: 42 }));
            await initFakeRepo();
            ...
            expect(helper.createComment).toHaveBeenCalledWith(mockRepo.full_name, 1, body);
        });
        it('should add comment without topic if not found', async () => {
            ...
            expect(helper.updateComment).not.toHaveBeenCalled();
            expect(helper.createComment).toHaveBeenCalledTimes(1);
            expect(helper.createComment).toHaveBeenCalledWith(mockRepo.full_name, 1, 'other-content');
        });
        it('should update comment with topic if found', async () => {
            helper.getComments.mockResolvedValueOnce(mockComments);
            helper.updateComment.mockResolvedValueOnce(util_1.partial({ id: 42 }));
            await initFakeRepo();
            const res = await gitea.ensureComment({
                number: 1,
                topic: 'some-topic',
            ...
            expect(helper.updateComment).toHaveBeenCalledWith(mockRepo.full_name, 1, body);
        });
    ...
    describe('getBranchPr', () => {
        it('should return existing pull request for branch', async () => {
            ...
            helper.searchPRs.mockResolvedValueOnce(mockPRs);
            await initFakeRepo();
            expect(await gitea.getBranchPr(mockPR.head.label)).toHaveProperty('number', mockPR.number);
        });
    ...
    describe('addReviewers', () => {
        it('should assign reviewers', async () => {
            expect.assertions(2);
            const mockPR = mockPRs[0];
            await expect(gitea.addReviewers(mockPR.number, ['me', 'you'])).resolves.not.toThrow();
