...
const ght = __importStar(require("./gitea-helper"));
describe('platform/gitea/gitea-helper', () => {
    ...
    describe('getRepoContents', () => {
        it('should call /api/v1/repos/[repo]/contents/[file] endpoint', async () => {
            ...
            const res = await ght.getRepoContents(mockRepo.full_name, mockContents.path);
        ...
        it('should support passing reference by query', async () => {
            httpMock
                .scope(baseUrl)
                .get(`/repos/${mockRepo.full_name}/contents/${mockContents.path}?ref=${mockCommitHash}`)
                .reply(200, { ...mockContents, contentString: undefined });
            const res = await ght.getRepoContents(mockRepo.full_name, mockContents.path, mockCommitHash);
            expect(res).toEqual(mockContents);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('should properly escape paths', async () => {
            ...
            const res = await ght.getRepoContents(mockRepo.full_name, otherMockContents.path);
    ...
    describe('mergePR', () => {
        it('should call /api/v1/repos/[repo]/pulls/[pull]/merge endpoint', async () => {
            httpMock
                ...
                .reply(200);
            const res = await ght.mergePR(mockRepo.full_name, mockPR.number, 'rebase');
            expect(res).toBeUndefined();
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    ...
    describe('addReviewers', () => {
        it('should call /api/v1/repos/[repo]/pulls/[pull]/requested_reviewers endpoint', async () => {
            httpMock
                .scope(baseUrl)
                .post(`/repos/${mockRepo.full_name}/pulls/${mockPR.number}/requested_reviewers`)
                .reply(200);
    ...
    describe('searchPRs', () => {
        ...
        it('should construct proper query parameters', async () => {
            httpMock
                .scope(baseUrl)
                .get(`/repos/${mockRepo.full_name}/pulls?state=open&labels=${mockLabel.id}&labels=${otherMockLabel.id}`)
                .reply(200, [mockPR]);
            const res = await ght.searchPRs(mockRepo.full_name, {
                state: types_1.PrState.Open,
    ...
    describe('unassignLabel', () => {
        it('should call /api/v1/repos/[repo]/issues/[issue]/labels/[label] endpoint', async () => {
            httpMock
                .scope(baseUrl)
                .delete(`/repos/${mockRepo.full_name}/issues/${mockIssue.number}/labels/${mockLabel.id}`)
                .reply(200);
            const res = await ght.unassignLabel(mockRepo.full_name, mockIssue.number, mockLabel.id);
            expect(res).toBeUndefined();
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    describe('createComment', () => {
        it('should call /api/v1/repos/[repo]/issues/[issue]/comments endpoint', async () => {
            httpMock
                .scope(baseUrl)
                .post(`/repos/${mockRepo.full_name}/issues/${mockIssue.number}/comments`)
                .reply(200, mockComment);
            const res = await ght.createComment(mockRepo.full_name, mockIssue.number, mockComment.body);
    ...
    describe('updateComment', () => {
        it('should call /api/v1/repos/[repo]/issues/comments/[comment] endpoint', async () => {
            ...
            const res = await ght.updateComment(mockRepo.full_name, mockComment.id, 'new-body');
    ...
    describe('deleteComment', () => {
        it('should call /api/v1/repos/[repo]/issues/comments/[comment] endpoint', async () => {
            httpMock
                .scope(baseUrl)
                .delete(`/repos/${mockRepo.full_name}/issues/comments/${mockComment.id}`)
                .reply(200);
    ...
    describe('createCommitStatus', () => {
        it('should call /api/v1/repos/[repo]/statuses/[commit] endpoint', async () => {
            ...
            const res = await ght.createCommitStatus(mockRepo.full_name, mockCommitHash, {
    ...
    describe('getCombinedCommitStatus', () => {
        it('should call /api/v1/repos/[repo]/commits/[branch]/statuses endpoint', async () => {
            ...
            const res = await ght.getCombinedCommitStatus(mockRepo.full_name, mockBranch.name);
...
//# sourceMappingURL=gitea-helper.spec.js.map