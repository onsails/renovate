...
describe('platform/bitbucket', () => {
    ...
    async function initRepoMock(config, repoResp, existingScope) {
        const repository = (config === null || config === void 0 ? void 0 : config.repository) || 'some/repo';
    ...
    describe('initPlatform()', () => {
        ...
        it('should show warning message if custom endpoint', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith('Init: Bitbucket Cloud endpoint should generally be https://api.bitbucket.org/ but is being configured to a different value. Did you mean to use Bitbucket Server?');
        });
        ...
        it('should warn for missing "profile" scope', async () => {
            ...
            expect(logger.warn).toHaveBeenCalledWith(`Bitbucket: missing 'account' scope for password`);
        });
    });
    describe('getRepos()', () => {
    ...
    describe('getBranchPr()', () => {
        it('bitbucket finds PR for branch', async () => {
            ...
            scope
                .get('/2.0/repositories/some/repo/pullrequests?state=OPEN&state=MERGED&state=DECLINED&state=SUPERSEDED&pagelen=50')
                .reply(200, { values: [pr] })
    ...
    describe('getBranchStatus()', () => {
        it('getBranchStatus 1', async () => {
            await initRepoMock();
            expect(await bitbucket.getBranchStatus('master', null)).toBe(types_1.BranchStatus.green);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('getBranchStatus 2', async () => {
            await initRepoMock();
            expect(await bitbucket.getBranchStatus('master', ['foo'])).toBe(types_1.BranchStatus.red);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('getBranchStatus 3', async () => {
            ...
            scope
                ...
                .get('/2.0/repositories/some/repo/commit/master_hash/statuses?pagelen=100')
                .reply(200, {
                values: [
                    {
                        key: 'foo',
            ...
            expect(await bitbucket.getBranchStatus('master', [])).toBe(types_1.BranchStatus.red);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('getBranchStatus 4', async () => {
            ...
            scope
                ...
                .get('/2.0/repositories/some/repo/commit/branch_hash/statuses?pagelen=100')
                .reply(200, {
                values: [
                    {
                        key: 'foo',
            ...
            expect(await bitbucket.getBranchStatus('branch', [])).toBe(types_1.BranchStatus.green);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('getBranchStatus 5', async () => {
            ...
            scope
                ...
                .get('/2.0/repositories/some/repo/commit/pending/branch_hash/statuses?pagelen=100')
                .reply(200, {
                values: [
                    {
                        key: 'foo',
            ...
            expect(await bitbucket.getBranchStatus('pending/branch', [])).toBe(types_1.BranchStatus.yellow);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('getBranchStatus 6', async () => {
            const scope = await initRepoMock();
            scope
                .get('/2.0/repositories/some/repo/refs/branches/branch-with-empty-status')
                .reply(200, {
                ...
                .get('/2.0/repositories/some/repo/commit/branch-with-empty-status/statuses?pagelen=100')
                .reply(200, {
                values: [],
            });
            expect(await bitbucket.getBranchStatus('branch-with-empty-status', [])).toBe(types_1.BranchStatus.yellow);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    describe('getBranchStatusCheck()', () => {
        ...
        it('getBranchStatusCheck 2', async () => {
            expect(await bitbucket.getBranchStatusCheck('master', 'foo')).toBe(types_1.BranchStatus.red);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    ...
    describe('findIssue()', () => {
        it('does not throw', async () => {
            ...
            scope
                .get('/2.0/repositories/some/repo/issues?q=title%3D%22title%22%20AND%20(state%20%3D%20%22new%22%20OR%20state%20%3D%20%22open%22)%20AND%20reporter.username%3D%22abc%22')
                .reply(200, {
                values: [
                    {
                        id: 25,
        ...
        it('returns null if no issues', async () => {
            ...
            scope
                .get('/2.0/repositories/some/empty/issues?q=title%3D%22title%22%20AND%20(state%20%3D%20%22new%22%20OR%20state%20%3D%20%22open%22)%20AND%20reporter.username%3D%22abc%22')
                .reply(200, {
                values: [],
            });
    ...
    describe('ensureIssue()', () => {
        it('updates existing issues', async () => {
            ...
            scope
                ...
                .reply(200);
            expect(await bitbucket.ensureIssue({ title: 'title', body: 'body' })).toEqual('updated');
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('creates new issue', async () => {
            const scope = await initRepoMock({ repository: 'some/empty' }, { has_issues: true });
            scope
                .get('/2.0/repositories/some/empty/issues?q=title%3D%22title%22%20AND%20(state%20%3D%20%22new%22%20OR%20state%20%3D%20%22open%22)%20AND%20reporter.username%3D%22abc%22')
                .reply(200, { values: [] })
                .get('/2.0/repositories/some/empty/issues?q=title%3D%22old-title%22%20AND%20(state%20%3D%20%22new%22%20OR%20state%20%3D%20%22open%22)%20AND%20reporter.username%3D%22abc%22')
                .reply(200, { values: [] })
                .post('/2.0/repositories/some/empty/issues')
                .reply(200);
            expect(await bitbucket.ensureIssue({
                title: 'title',
    ...
    describe('ensureCommentRemoval()', () => {
        it('does not throw', async () => {
            expect(await bitbucket.ensureCommentRemoval({ number: 3, topic: 'topic' })).toMatchSnapshot();
        });
    });
    describe('getPrList()', () => {
        it('exists', () => {
