...
describe('platform/gitlab', () => {
    ...
    beforeEach(async () => {
        ...
        jest.resetModules();
        jest.resetAllMocks();
        gitlab = await Promise.resolve().then(() => __importStar(require('.')));
    ...
    describe('initPlatform()', () => {
        ...
        it(`should default to gitlab.com`, async () => {
            ...
            expect(await gitlab.initPlatform({
                token: 'some-token',
                endpoint: undefined,
            })).toMatchSnapshot();
            expect(httpMock.getTrace()).toMatchSnapshot();
    ...
    describe('getRepos', () => {
        it('should throw an error if it receives an error', async () => {
            httpMock
                .scope(gitlabApiHost)
                .get('/api/v4/projects?membership=true&per_page=100&with_merge_requests_enabled=true&min_access_level=30')
    ...
    async function initRepo(repoParams = {
        repository: 'some/repo',
        localDir: '',
    }, repoResp = null, scope = httpMock.scope(gitlabApiHost)) {
        const repo = repoParams.repository;
        const justRepo = repo.split('/').slice(0, 2).join('/');
        scope.get(`/api/v4/projects/${encodeURIComponent(repo)}`).reply(200, repoResp || {
    ...
    describe('initRepo', () => {
        ...
        it('should throw an error if repository is archived', async () => {
            ...
            await expect(gitlab.initRepo({
                repository: 'some/repo',
                localDir: '',
            })).rejects.toThrow(error_messages_1.REPOSITORY_ARCHIVED);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
        it('should throw an error if repository is a mirror', async () => {
            ...
            await expect(gitlab.initRepo({
                repository: 'some/repo',
                localDir: '',
            })).rejects.toThrow(error_messages_1.REPOSITORY_MIRRORED);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    ...
    describe('getBranchPr(branchName)', () => {
        ...
        it('should return the PR object', async () => {
            ...
            scope
                ...
                .get('/api/v4/projects/some%2Frepo/merge_requests/91?include_diverged_commits_count=1')
                .reply(200, {
                iid: 91,
                title: 'some change',
                state: 'opened',
                ...
                .get('/api/v4/projects/some%2Frepo/repository/commits/0d9c7726c3d628b7e28af234595cfd20febdbf8e/statuses')
                .reply(200, []);
            const pr = await gitlab.getBranchPr('some-branch');
    ...
    describe('getBranchStatus(branchName, requiredStatusChecks)', () => {
        ...
        it('throws repository-changed', async () => {
            ...
            await initRepo();
            await expect(gitlab.getBranchStatus('somebranch', [])).rejects.toThrow(error_messages_1.REPOSITORY_CHANGED);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    describe('getBranchStatusCheck', () => {
        it('returns null if no results', async () => {
            ...
            scope
                .get('/api/v4/projects/some%2Frepo/repository/commits/0d9c7726c3d628b7e28af234595cfd20febdbf8e/statuses')
                .reply(200, []);
            const res = await gitlab.getBranchStatusCheck('somebranch', 'some-context');
            expect(res).toBeNull();
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    ...
    describe('setBranchStatus', () => {
        it.each([types_1.BranchStatus.green, types_1.BranchStatus.yellow, types_1.BranchStatus.red])('sets branch status %s', async (state) => {
            const scope = await initRepo();
            scope
                .post('/api/v4/projects/some%2Frepo/statuses/0d9c7726c3d628b7e28af234595cfd20febdbf8e')
                .reply(200, {})
                .get('/api/v4/projects/some%2Frepo/repository/commits/0d9c7726c3d628b7e28af234595cfd20febdbf8e/statuses')
                .reply(200, []);
    ...
    describe('findIssue()', () => {
        it('returns null if no issue', async () => {
            httpMock
                .scope(gitlabApiHost)
                .get('/api/v4/projects/undefined/issues?per_page=100&author_id=undefined&state=opened')
                .reply(200, [
                {
                    iid: 1,
                    title: 'title-1',
    ...
    describe('addAssignees(issueNo, assignees)', () => {
        ...
        it('should warn if more than one assignee', async () => {
            httpMock
                ...
                .put('/api/v4/projects/undefined/merge_requests/42?assignee_id=123')
                .reply(200)
                .put('/api/v4/projects/undefined/merge_requests/42?assignee_ids[]=123&assignee_ids[]=124')
    ...
    async function initPlatform(gitlabVersion) {
        httpMock
            .scope(gitlabApiHost)
            .get('/api/v4/user')
            .reply(200, {
    ...
    describe('getPr(prNo)', () => {
        it('returns the PR', async () => {
            httpMock
                .scope(gitlabApiHost)
                .get('/api/v4/projects/undefined/merge_requests/12345?include_diverged_commits_count=1')
                .reply(200, {
                id: 1,
                iid: 12345,
                title: 'do something',
        ...
        it('returns the mergeable PR', async () => {
            const scope = await initRepo();
            scope
                .get('/api/v4/projects/some%2Frepo/merge_requests/12345?include_diverged_commits_count=1')
                .reply(200, {
                id: 1,
                iid: 12345,
                title: 'do something',
...
`;
    ...
    describe('deleteLabel(issueNo, label)', () => {
        it('should delete the label', async () => {
            httpMock
                .scope(gitlabApiHost)
                .get('/api/v4/projects/undefined/merge_requests/42?include_diverged_commits_count=1')
                .reply(200, {
                id: 1,
                iid: 12345,
                title: 'some change',
    ...
    describe('getJsonFile()', () => {
        it('returns file content', async () => {
            ...
            const scope = await initRepo();
            scope
                .get('/api/v4/projects/some%2Frepo/repository/files/dir%2Ffile.json?ref=master')
                .reply(200, {
        ...
        it('returns null on errors', async () => {
            const scope = await initRepo();
            scope
                .get('/api/v4/projects/some%2Frepo/repository/files/file.json?ref=master')
                .replyWithError('some error');
