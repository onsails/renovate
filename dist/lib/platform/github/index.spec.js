...
describe('platform/github', () => {
    ...
    beforeEach(async () => {
        ...
        github = await Promise.resolve().then(() => __importStar(require('.')));
        hostRules = util_1.mocked(await Promise.resolve().then(() => __importStar(require('../../util/host-rules'))));
        jest.mock('../../util/git');
        git = util_1.mocked(await Promise.resolve().then(() => __importStar(require('../../util/git'))));
    ...
    const graphqlOpenPullRequests = fs_extra_1.default.readFileSync('lib/platform/github/__fixtures__/graphql/pullrequest-1.json', 'utf8');
    const graphqlClosedPullRequests = fs_extra_1.default.readFileSync('lib/platform/github/__fixtures__/graphql/pullrequests-closed.json', 'utf8');
    describe('initPlatform()', () => {
        it('should throw if no token', async () => {
            await expect(github.initPlatform({})).rejects.toThrow('Init: You must configure a GitHub personal access token');
        });
        ...
        it('should support gitAuthor and username', async () => {
            expect(await github.initPlatform({
                token: 'abc123',
    ...
    function initRepoMock(scope, repository) {
    ...
    function forkInitRepoMock(scope, repository, forkedRepo) {
        scope
            // repo info
            .post(`/graphql`)
            .reply(200, {
    ...
    describe('initRepo', () => {
        ...
        it('should throw error if renamed', async () => {
            ...
            await expect(github.initRepo({
                repository: 'some/repo',
            })).rejects.toThrow(error_messages_1.REPOSITORY_RENAMED);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    describe('getRepoForceRebase', () => {
        ...
        it('should throw 401', async () => {
            httpMock
                ...
                .get('/repos/undefined/branches/undefined/protection')
                .reply(401);
            await expect(github.getRepoForceRebase()).rejects.toThrowErrorMatchingSnapshot();
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    });
    ...
    describe('getBranchStatusCheck', () => {
        it('returns state if found', async () => {
            ...
            initRepoMock(scope, 'some/repo');
            scope
                .get('/repos/some/repo/commits/0d9c7726c3d628b7e28af234595cfd20febdbf8e/statuses')
                .reply(200, [
                {
                    context: 'context-1',
                    state: 'success',
            ...
            const res = await github.getBranchStatusCheck('renovate/future_branch', 'context-2');
            expect(res).toEqual(types_1.BranchStatus.yellow);
            expect(httpMock.getTrace()).toMatchSnapshot();
        });
    ...
    describe('setBranchStatus', () => {
        ...
        it('sets branch status', async () => {
            ...
            scope
                ...
                .post('/repos/some/repo/statuses/0d9c7726c3d628b7e28af234595cfd20febdbf8e')
                .reply(200)
    ...
    describe('ensureCommentRemoval', () => {
        it('deletes comment by topic if found', async () => {
            ...
            scope
                ...
                .reply(200);
            await github.initRepo({ repository: 'some/repo', token: 'token' });
    ...
    describe('getPrBody(input)', () => {
        it('returns updated pr body', () => {
            const input = 'https://github.com/foo/bar/issues/5 plus also [a link](https://github.com/foo/bar/issues/5)';
