...
async function getJsonFile(fileName) {
    try {
        const json = await azureHelper.getFile(config.repoId, fileName, config.defaultBranch);
...
async function initRepo({ repository, localDir, cloneSubmodules, }) {
    var _a, _b;
    ...
    config.defaultMergeMethod = await azureHelper.getMergeMethod(repo.id, names.project);
    config.mergeMethods = {};
...
async function findPr({ branchName, prTitle, state = types_1.PrState.All, }) {
    ...
    try {
        ...
        prsFiltered = prs.filter((item) => item.sourceRefName === util_1.getNewBranchName(branchName));
        if (prTitle) {
...
async function getStatusCheck(branchName) {
    const azureApiGit = await azureApi.gitApi();
    const branch = await azureApiGit.getBranch(config.repoId, util_1.getBranchNameWithoutRefsheadsPrefix(branchName));
...
async function getBranchStatusCheck(branchName, context) {
    ...
    for (const check of res) {
        if (util_1.getGitStatusContextCombinedName(check.context) === context) {
...
async function createPr({ sourceBranch, targetBranch, prTitle: title, prBody: body, labels, draftPR = false, platformOptions, }) {
    var _a;
    const sourceRefName = util_1.getNewBranchName(sourceBranch);
    const targetRefName = util_1.getNewBranchName(targetBranch);
    ...
    if (platformOptions === null || platformOptions === void 0 ? void 0 : platformOptions.azureAutoComplete) {
    ...
    return util_1.getRenovatePRFormat(pr);
}
...
async function setBranchStatus({ branchName, context, description, state, url: targetUrl, }) {
    ...
    const statusToCreate = {
        description,
        context: util_1.getGitStatusContextFromCombinedName(context),
...
async function mergePr(pullRequestId, branchName) {
    var _a;
    logger_1.logger.debug(`mergePr(${pullRequestId}, ${branchName})`);
    const azureApiGit = await azureApi.gitApi();
    const pr = await azureApiGit.getPullRequestById(pullRequestId, config.project);
    const mergeMethod = (_a = config.mergeMethods[pr.targetRefName]) !== null && _a !== void 0 ? _a : (config.mergeMethods[pr.targetRefName] = await azureHelper.getMergeMethod(config.repoId, config.project, pr.targetRefName));
    const objToUpdate = {
    ...
    try {
        await azureApiGit.updatePullRequest(objToUpdate, config.repoId, pullRequestId);
        return true;
    }
    catch (err) {
