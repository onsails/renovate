...
async function getUpdatedPackageFiles(config) {
    ...
    for (const upgrade of config.upgrades) {
        ...
        if (upgrade.updateType === 'lockFileMaintenance') {
            ...
            const bumpPackageVersion = manager_1.get(manager, 'bumpPackageVersion');
            const updateDependency = manager_1.get(manager, 'updateDependency');
            if (!updateDependency) {
                let res = await auto_replace_1.doAutoReplace(upgrade, existingContent, reuseExistingBranch);
                if (res) {
